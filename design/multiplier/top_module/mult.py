...
from design.multiplier.pp_generator import PartialProductGenerator, AndMultPPG, BoothRadix4MultPPG
from design.multiplier.comp_tree import CompTreeGraphView
from design.adder.add_config import AdderConfig, DefaultAdderConfig
from utils import mkdir, execute, remove, if_exist

class MultTop(CompTreeTopModule):
    """
    ...
    def __init__(self, comp_tree: CompTreeGraphView, final_adder: AdderConfig) -> None:
        ...
        assert isinstance(comp_tree.pp_gen, (AndMultPPG, BoothRadix4MultPPG))
...
""" % (
    ...
    m + n - 1,
        ...
        # final adder
        if isinstance(self.final_adder, DefaultAdderConfig):
...
"""
    ...
    #10;
    if (c !== gold) begin
        $display("a=%d, b=%d, c=%b, gold=%b", a, b, c, gold);
        $fatal(1);
    end
"""
        codes += """
...
"""
    ...
    def run_revsca(self, rundir: str = None) -> bool:
        """
            Use yosys and revsca to quickly verify multipliers, return if the simulation is successful
        """
        assert self.pp_gen.multiplicand_bit == self.pp_gen.multiplier_bit, "Only support square multipliers"

        mkdir(rundir)
        verilog_path = os.path.join(rundir, 'top.v')

        with open(verilog_path, 'w') as f:
            f.write(self.generate_verilog(use_clock=False))

        yosys_cmd = "cd %s && yosys -p 'read_verilog %s; synth -flatten; aigmap; write_aiger top.aig'" % (rundir, verilog_path)
        execute(yosys_cmd, verbose=False, wait=True)
        if not if_exist(os.path.join(rundir, 'top.aig')):
            return False

        revsca_cmd = "cd %s && revsca top.aig top.out %s" % (rundir, '-u' if self.unsigned else '-s')
        execute(revsca_cmd, verbose=False, wait=True)
        if not if_exist(os.path.join(rundir, 'top.out')):
            return False
        
        cmd = "cat %s | grep 'The multiplier is correct!' | wc -l" % os.path.join(rundir, 'top.out')
        ret = subprocess.run(["/bin/bash", "-c", cmd], capture_output=True, text=True)
        num_correct = int(ret.stdout.strip())        
