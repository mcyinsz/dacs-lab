...
class GenusInnovusFlow():
    ...
    def get_floorplan_area(self) -> float:
        report_path = os.path.join(self.rundir, 'innovus-rundir', 'reports', 'floorplan_area.rpt')
        with open(report_path, 'r') as f:
            area = float(f.readlines()[0])
        return area

    def get_drv(self) -> int:
        report_path = os.path.join(self.rundir, 'innovus-rundir', 'reports', 'chipdone_drv.rpt')
        with open(report_path, 'r') as f:
            for line in f.readlines():
                if line.startswith('  Total Violations'):
                    drv = int(re.findall(r'\d+', line)[0])
